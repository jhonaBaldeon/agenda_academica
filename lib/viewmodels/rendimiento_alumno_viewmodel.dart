...
class RendimientoAlumnoViewModel extends ChangeNotifier {
  ...
  Future<void> calcularEstadisticasPorAlumno(String alumnoId) async {
    _setLoading(true);
    _error = null;

    try {
      // Obtener datos del alumno
      final alumno = await _alumnoRepository.getAlumnoById(alumnoId);
      ...
      for (final curso in cursosSnapshot) {
        // Obtener seguimientos del alumno en este curso
        final seguimientosSnapshot = await _seguimientoRepository
            .getSeguimientosByAlumnoAndCurso(alumnoId, curso.id)
            .first;
        ...
        estadisticasCursosList.add(
          EstadisticaCurso(
            ...
            color: curso.color,
            totalAlumnos: 1, // Solo un alumno (el seleccionado)
